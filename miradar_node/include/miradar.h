...
#define STDEF_INIT_ETH_IP           "192.168.0.12"
#define STDEF_INIT_ETH_PORT         9611
...
#define TM_RECEIVE_FIN              10

class MrUdp {
private:
    typedef struct {
        int nFlagUdpConnected;
	    struct sockaddr_in UdpRemoteAdr;
        struct timeval TmRxFin;
        int nMrPort;
    } tagLocalSocketParameter;

    tagLocalSocketParameter *LSP;
    ...
    int DiffTime(struct timeval *TmInStart, struct timeval *TmOutNow) {
    	struct timeval TmNow;
	    struct timezone TzNow;
	    unsigned long lStart, lNow, lDiff;
	    int nmsec = 0;

	    gettimeofday(&TmNow, &TzNow);
	    if(TmOutNow!=NULL) {
		    (*TmOutNow) = TmNow;
	    }
	
	    lStart = (((unsigned long)(TmInStart->tv_sec)) * 1000000) + ((unsigned long)(TmInStart->tv_usec));
	    lNow = (((unsigned long)(TmNow.tv_sec)) * 1000000) + ((unsigned long)(TmNow.tv_usec));
        if(lNow<lStart) {
            lNow += (60 * 1000000);
        }
	
	    lDiff = lNow - lStart;
	    nmsec = (int)(lDiff / 1000);
...
public:
    ...
    MrUdp(void) {
        LSP = new tagLocalSocketParameter;
        memset(LSP, 0, sizeof(tagLocalSocketParameter));
        (LSP->nMrPort) = STDEF_INIT_ETH_PORT;
        fd = 0;
    }

   ~MrUdp() {
    	if((LSP->nFlagUdpConnected)==1) {
            close(fd);
            (LSP->nFlagUdpConnected) = 0;
        }
        delete(LSP);
    }

    int CommInit(std::string deviceFile) {
    	if((LSP->nFlagUdpConnected)==0) {
        	struct sockaddr_in udpAddr;
	        memset(&udpAddr, 0, sizeof(udpAddr));
	        udpAddr.sin_addr.s_addr = INADDR_ANY;
	        udpAddr.sin_port = htons(LSP->nMrPort);
	        udpAddr.sin_family = AF_INET;
	        fd = socket(AF_INET, SOCK_DGRAM, 0);
	        bind(fd, (struct sockaddr *)&udpAddr, sizeof(udpAddr));

            int nNonBlock = 1;
            ioctl(fd, FIONBIO, &nNonBlock);

	        memset(&(LSP->UdpRemoteAdr), 0, sizeof(LSP->UdpRemoteAdr));
	        (LSP->UdpRemoteAdr.sin_addr.s_addr) = inet_addr((char*)deviceFile.c_str());
	        (LSP->UdpRemoteAdr.sin_port) = htons(LSP->nMrPort);
	        (LSP->UdpRemoteAdr.sin_family) = AF_INET;

            (LSP->nFlagUdpConnected) = 1;
        }
    ...
    int CommTx(char *bpBuf, int nLen) {
		sendto(fd, bpBuf, nLen, 0, ((struct sockaddr *)(&(LSP->UdpRemoteAdr))), sizeof(LSP->UdpRemoteAdr));
        return(0);
    }
    ...
    int CommRx(char *bpBuf, int nBufSize, int nTmOut) {
        ...
        while (nReqQuit == 0) {
			nErr = recv(fd, bp1, (nBufSize-nRxLen), 0);
            //------- received
            if (0 < nErr) {
                bp1 += nErr;
                nRxLen += nErr;
                GetTime(&(LSP->TmRxFin));
                nTmOut = TM_RECEIVE_FIN;
                continue;
            }
            //------- no received
            nDiff = DiffTime(&(LSP->TmRxFin), NULL);
            if(nTmOut<=nDiff) {
    		    nReqQuit = 1;
            }
            //--------
...
public:
    ...
    void setUdp(MrUdp& udp) { comm = udp; }
